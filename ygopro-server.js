...
(function() {
  var CLIENT_get_authorize_key, CLIENT_get_kick_reconnect_target, CLIENT_heartbeat_register, CLIENT_heartbeat_unregister, CLIENT_import_data, CLIENT_is_able_to_kick_reconnect, CLIENT_is_able_to_reconnect, CLIENT_is_banned_by_mc, CLIENT_is_player, CLIENT_kick, CLIENT_kick_reconnect, CLIENT_pre_reconnect, CLIENT_reconnect, CLIENT_reconnect_register, CLIENT_reconnect_unregister, CLIENT_send_pre_reconnect_info, CLIENT_send_reconnect_info, CLIENT_send_replays, CORES_list, ROOM_all, ROOM_bad_ip, ROOM_ban_player, ROOM_clear_disconnect, ROOM_connected_ip, ROOM_find_by_game_id, ROOM_find_by_name, ROOM_find_by_pid, ROOM_find_by_port, ROOM_find_by_title, ROOM_find_or_create_by_name, ROOM_find_or_create_random, ROOM_player_flee, ROOM_player_get_score, ROOM_player_lose, ROOM_player_win, ROOM_players_banned, ROOM_players_oppentlist, ROOM_players_scores, ROOM_unwelcome, ROOM_validate, Room, SERVER_clear_disconnect, SOCKET_flush_data, _, addCallback, badwords, ban_user, botServer, bunyan, chat_color, config, crypto, date, default_data, delete_outdated_cores, dialogues, disconnect_list, duel_log, exec, execFile, fs, geoip, get_latest_core_path, get_memory_usage, http, http_server, https, https_server, import_datas, imported, j, l, len, len1, len2, lflists, list, loadJSON, load_dialogues, load_tips, log, long_resolve_cards, m, memory_usage, merge, moment, net, options, os, path, plugin_filename, plugin_list, plugin_path, rebooted, ref, ref1, release_disconnect, report_to_big_brother, request, requestListener, rmdir, roomlist, setting_change, setting_save, settings, spawn, spawnSync, tips, update_core_paths, url, wait_room_start, ygopro,
    splice = [].splice;
  ...
  ROOM_find_or_create_random = global.ROOM_find_or_create_random = function(type, player_ip) {
    ...
    result = _.find(ROOM_all, function(room) {
      return room && room.random_type !== '' && room.duel_stage === ygopro.constants.DUEL_STAGE.BEGIN && ((type === '' && (room.random_type === 'S' || (settings.modules.random_duel.blank_pass_match && room.random_type !== 'T'))) || room.random_type === type) && room.get_playing_player().length < max_player && (settings.modules.random_duel.no_rematch_check || room.get_host() === null || room.get_host().ip !== ROOM_players_oppentlist[player_ip]) && (playerbanned === room.deprecated || type === 'T');
    });
    if (result) {
  ...
  Room = class Room {
    constructor(info) {
      ...
      this.game_id = ((Math.floor(new Date().getTime() / 4) + Math.floor(Math.random() * 100000000)) & 0xffffffff) >>> 0;
      ROOM_all.push(this);
      try {
        //if lflists.length
        ...
        this.process.on('error', (err) => {
          _.each(this.players, function(player) {
            return ygopro.stoc_die(player, err);
          });
          this.delete();
        });
    ...
    connect(client) {
      ...
      if (this.established) {
        if (this.duel_stage === ygopro.constants.DUEL_STAGE.BEGIN && settings.modules.http.websocket_roomlist) {
    ...
    disconnect(client, error) {
      ...
      if (client.is_post_watcher) {
        ...
        if (this.players.length && client.is_host && !(this.duel_stage === ygopro.constants.DUEL_STAGE.BEGIN && client.pos <= 3)) {
  ...
  ygopro.stoc_follow('JOIN_GAME', false, function(buffer, info, client, server, datas) {
    ...
    if (!room.watcher && !room.hostinfo.no_watch) {
  ...
  ygopro.stoc_follow('DUEL_START', false, function(buffer, info, client, server, datas) {
    var deck_text, len2, len3, m, n, player, ref2, ref3, room;
    room = ROOM_all[client.rid];
    if (!(room && !client.reconnecting)) {
      return;
    }
  ...
  ygopro.ctos_follow('UPDATE_DECK', true, function(buffer, info, client, server, datas) {
    var buff_main, buff_side, i, oppo_pos, room, win_pos;
  ...
  ygopro.stoc_follow('TIME_LIMIT', true, function(buffer, info, client, server, datas) {
    ...
    if (!(settings.modules.heartbeat_detection.enabled && room.duel_stage === ygopro.constants.DUEL_STAGE.DUELING)) {
      return;
    }
    check = false;
    if (room.hostinfo.mode !== 2) {
  ...
  if (settings.modules.heartbeat_detection.enabled) {
    setInterval(function() {
      ...
      for (m = 0, len2 = ROOM_all.length; m < len2; m++) {
        room = ROOM_all[m];
        if (room && room.duel_stage !== ygopro.constants.DUEL_STAGE.BEGIN && (room.hostinfo.time_limit === 0 || room.duel_stage !== ygopro.constants.DUEL_STAGE.DUELING)) {
          ref2 = room.get_playing_player();
          for (n = 0, len3 = ref2.length; n < len3; n++) {
            player = ref2[n];
  ...
  if (settings.modules.http) {
    ...
    requestListener = function(request, response) {
      var parseQueryString, player, room, roomsjson, u;
                    ...
                    for (n = 0, len3 = ref2.length; n < len3; n++) {
                      ...
                      if (player.pos != null) {
                        results1.push({
                          id: (-1).toString(),
                          name: player.name,
                          ip: settings.modules.http.show_ip && !player.is_local ? player.ip.slice(7) : null,
